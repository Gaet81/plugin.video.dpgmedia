...
class Channels:
    ...
    def show_channels(self,module):
        ...
        for channel in channels:
            ...
            if channel.epg:
                label = title + '[COLOR lightgray] | {title} ({start} - {end})[/COLOR]'.format(
            ...
            listing.append(kodiutils.TitleItem(
                title=label,
                path=kodiutils.url_for('show_channel_menu', module=module, channel=channel.key),
                art_dict=dict(
                    icon=icon,
                    thumb=icon,
                    fanart=fanart,
    ...
    def show_channel_menu(self, module, key):
        ...
        if channel.epg:
            label = label + '[COLOR lightgray] | {title} ({start} - {end})[/COLOR]'.format(
                title=channel.epg[0].title,
                start=channel.epg[0].start.strftime('%H:%M'),
                end=channel.epg[0].end.strftime('%H:%M'))
